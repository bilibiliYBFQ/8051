...
//参数说明：Command[IN]:RC522命令字  PCD_TRANSCEIVE为发送并接收数据
//          pInData[IN]:通过RC522发送到卡片的数据ucComMF522Buf
...
char PcdAnticoll(unsigned char *pSnr)
{
    ...
    ClearBitMask(Status2Reg,0x08);//解除加密状态
    WriteRawRC(BitFramingReg,0x00);//传输最后一个字节所有位
    ClearBitMask(CollReg,0x80);//初始化碰撞检测寄存器
