...
void show_num48(uint8 screen,uint8 page,uint8 column,uint8 *p)//(屏幕，列，行，数据地址)
{
	uint8 i;
	SelectScreen(screen);
	Set_page(page);
	...
	Set_page(page+1);
	Set_column(column);
	for(i=0;i<24;i++)	//采用48*24的字模,"小四号字"
	{
		write_LCD_data(p[i+24]);
	}
